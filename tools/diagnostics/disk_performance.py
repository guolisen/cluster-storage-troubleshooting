...
def run_disk_readonly_test(node_name: str, device_path: str, 
    ...
    try:
        ...
        # Using dd with if=device and of=/dev/null to perform read-only operations
        cmd = (
        ...
        report = [
            ...
            f"Total Data Read: {read_bytes} bytes ({read_bytes / (1024**3):.2f} GB)",
            f"Average Read Speed: {read_speed}",
            f"Read Errors Detected: {len(read_errors)}",
        ]
...
def test_disk_io_performance(node_name: str, device_path: str, 
    ...
    try:
        ...
        valid_test_types = ["read", "write", "randread", "randwrite"]
